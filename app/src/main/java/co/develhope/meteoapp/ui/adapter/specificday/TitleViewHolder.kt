...
class TitleViewHolder(private val view: View) : RecyclerView.ViewHolder(view) {
    ...
    fun bind(detailsTitle: SpecyfDayScreenItem.DetailsTitle) {
        textViewTitle.text =  "PIPPO"//detailsTitle.titleForecast.titleGiorno
        textViewTitle2.text = itemView.context.getString(R.string.data_today, detailsTitle.titleForecast.date.dayOfWeek.name, detailsTitle.titleForecast.date.dayOfMonth.toString(),detailsTitle.titleForecast.date.month.name)
